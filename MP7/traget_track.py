...
class Solution():
    def __init__(self, path, title):
        ...
        cv.imwrite('result/' +'original.jpg', self.img)
    ...
    def targetTracking(self, mode='SSD'):
        ...
        if len(points) == 2:
            ...
            cv.imwrite('result/' + mode + '/' + 'cropped_img.jpg', T)
            cv.waitKey(0)
        ...
        for s in range(1,501):
            ...
            # save image
            cv.imwrite('result/' + mode + '/' + num + '.jpg', result)
            print(s)
...
# Experiment
girl = Solution('video/image_girl/','girl')
...
girl.targetTracking(mode='NCC')
girl.createVideo('tracked_NCC', 'result/SSD/', 15)